...
func Combine(srcArray []DbPojo, selector int) [][]DbPojo {
	timeStart := time.Now()
	indexes := combineResult(len(srcArray), selector)
	result := findResultByIndexs(srcArray, indexes)
	timeEnd := time.Now()
...
func combineResult(n int, m int) [][]int {
	...
	result := make([][]int, 0, combineCount(n, m))
...
//根据索引号数组得到元素数组
func findResultByIndexs(srcArray []DbPojo, indexs [][]int) [][]DbPojo {
	if len(indexs) == 0 {
		return [][]DbPojo{}
	}

	result := make([][]DbPojo, len(indexs))

	for i, v := range indexs {
		line := make([]DbPojo, 0)
		for j, v2 := range v {
			if v2 == 1 {
				line = append(line, srcArray[j])
			}
		}
		result[i] = line
	}
...
func permutation(n int, m int) int {
	return factorial(n) / factorial(n-m)
}

//数学方法计算组合数(从n中取m个数)
func combineCount(n int, m int) int {
